...
from messages import TEXT_B_FULFILL_AWARDS, TEXT_SUBSCRIBE
...
class FulfillAwards(StatesGroup):
    award_id = State()

@router.message(F.text == TEXT_B_FULFILL_AWARDS)
async def cmd_add_awards_1(message: Message, state: FSMContext):
    await state.clear()
    await state.set_state(FulfillAwards.award_id)
    await message.answer(f'<pre>Ведьмин писаръ</pre> \nДавайте закроем награду. Напиши номер её, что требует закрытия.', parse_mode=ParseMode.HTML, 
                         reply_markup=choice_kb())
    
@router.message(FulfillAwards.award_id, AllowedContentType(['text']), IsInt())
async def cmd_add_awards_2(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_channel_status.status not in ['left','kicked']:
        ...
        await database.fulfill_award(int(data['award_id']))
